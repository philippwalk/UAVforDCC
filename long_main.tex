...
positions are degenerated to scalars, i.e., $p_n\in[s, t], \forall n\in\{1, \dots, N\}$.  \philippstart If we shift the
...
$[s,t]$ to $[0,A]$ where $A=t-s$ without changing the average power consumption. Hence we will only consider here target
areas of the form $[0,A]$ with $A>0$.  \philippend
...
\begin{lemma}\label{lem:ggam}
  Let $A>0$. Then the $1-$level parameter optimized quantizer for a uniform source distribution is given by
  The function 
  %
  \begin{align}
...
\begin{proof}
  ...
  %
  \begin{align}
    F'(h)  &=\int_{0}^{1}\big( 2\gam (\ome^2+h^2)^{\gam-1} - h^{-2}(\ome^2+h^2)^{\gam}\big) d\ome\label{eq:fprime} \\
           &= \int_0^{1} (\ome^2+h^2)^{\gam-1}( 2\gam\!-\!1\!-\!h^{-2}\ome^2)d\ome
            =\frac{1}{h^{2\gam-1}}\int_{0}^{\frac{1}{h}} (\ome^2+1)^{\gam-1}(2\gam-1-\ome^2)d\ome\notag.
  \end{align}
  %
