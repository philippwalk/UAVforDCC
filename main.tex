...
\subsection{Static UAV Deployment: Local Optimality}
%
...
\begin{align}
  ...
  h_n^{-1}(\Norm{p_n-\vome}^2 +h_n^2)^{\gam}\df(\vome)d\vome\label{eq:phoptlocal} 
\end{align}
%
where $\Vor_n(\vp,h)$ are the Möbius regions given in \eqref{eq:moebius} for each fixed 
$\vq=(\vp,h)$.


\begin{theorem}\label{thm:necessarycond}
  Let $\alp \geq 1$ be fixed. Then a point $\vq^*=(\vp^*,h^*)$ with Voronoi tessellation
  $\Vor^*=\Vor(\vp^*,h^*)=(\Vor_1^*,\dots,\Vor^*_N)$ given by \lemref{lem:moebiusdia} is a critical point of
...
%
\begin{proof}
  For a  proof see  \appref{app:UniformScalarQuantizer}.  See also  the scalar quantization theorem in \cite{GN98} or
...
\begin{theorem}\label{thm:commonheight}
  ...
  \begin{align}
   ...
   \ \  g(5) = \sqrt{\frac{ (\frac{32}{7})^{1/3}-1}{5}}
    %  \Pbar(\qLoc)= \frac{A}{\sqrt{3}N}\quad\text{with}\quad 
  \end{align}
...
\end{align}
%
Then the optimal ground positions $x_1,\dots,x_n^*$ need to satisfy by \thmref{thm:necessarycond}
%
\begin{align}
  0 = \int_{\Vor_n(\vx^*)} (x_n^*-\ome) ((x_n^*-\ome)^2+h^2)^{\gam-1}d\ome \quad,\quad 1\leq n\leq N\label{eq:vornx}
\end{align}
%
Since the Voronoi regions are intervals given by \lemref{lem:moebiusdia} and \eqref{eq:Vorn} as
%
\begin{align}
...
\end{align}
%
Hence, by the same argument as in the proof \appref{app:UniformScalarQuantizer} of \lemref{lem:UniformScalarQuantizer}
