...
\section{Proof of \lemref{lemma:allActive}}\label{app:proof_lemma_active}
  ...
  generalized Voronoi regions as the optimal partition (tessellation\footnote{Since we take here the continuous case,
    ...
    and target area $\Ome$.  To see this\footnote{We use the same argumentation as in the prove of
      \cite[Prop.1]{Erdem}.}, let us start with any quantizer $(\vP,\vh,\Rset)$ for $\Ome$ yielding to the average
      distortion 
  %
  \begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  $\Vor_n^*=[b^*_{n-1},b^*_n]$. We know by \lemref{lem:ggam} that the optimal quantizer $q_n^*$ for some closed interval
...
\section{Proof of \thmref{thm:commonheight}}\label{sec:proof_theorem} 
  ...
  $2(\pGlob_n-b_{n-1}^*)/\mu_n=1=-2(\pGlob_n-b_{n}^*)/\mu_n$.  We do not know the value of $\tilde{h}_n$ and $\mu_n$ but
  we know that $\mu_n>0$ and $\sum_{n=1}^N\mu_n=A$ by \lemref{lemma:allActive}
  %
  \begin{align}
    \min_{\vh>0} \AvDis(\vQ^*,\vh)=\AvDis(\vQ^*,\vh^*,\Vor^*)
  \end{align}
  %
  Since we split $h_n$ in $\tilde{h}_n$ and $\mu_n$ where $\sum_{n=1}^N \mu_n=A$ we get
  %
  \begin{align}
    \min_{\vh>0} \AvDis(\vQ^*,\vh)=\min_{\tilde{h}>0}\min_{\mu_n, \sum_n\mu_n=A}
  \end{align}
  it
  is the optimal parameter to achieve the minimal average distortion for $x_n^*$ and $\Vor_n^*$. Hence, by minimizing
  $\AvDis(\vP^*,\tilde{\vh},\Vor^*)$ over $\tilde{\vh}$ must yield the optimal parameter $\vh^*$ for the optimal
  quantization point $\vq_n^*$. 
  %
  \begin{align}
    \frac{1}{2^{2\gam-1}A}
    \sum_{n=1}^N \mu_n^{2\gam}\cdot\left(\min_{\tilde{h}_n>0}\int_0^1 
      \frac{(\ome^2+\tilde{h}^2_n)^{\gam}}{\tilde{h}_n}d \ome\right)
