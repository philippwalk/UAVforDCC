...
\begin{abstract}
  ...
  given source sample to any given reproduction point of the quantizer. We will in this work regard distortion functions
  which are weighted for each reproduction point resulting in a heterogeneous quantization problem with
  non-convex quantization sets, as used in deployment problems of sensor networks. Whereas, in such problems, the
  average distortion is minimized for given weights, we will optimize here the quantization problem over all positive 
  weights.
  For a uniform source distribution in one-dimension we derived the unique minimizer, given as the uniform scalar
  quantizer with an optimal common weight. By numerical simulations this result extends to two-dimensions where
  asymptotically the optimal quantizer is the hexagonal lattice with common weights. As an application we will
  demonstrate the optimal deployment of unmanned aerial vehicles (UAV) to provide a wireless communication to ground users
  under minimal communication power cost. Here, the optimal positive weights refer to the optimal flight heights of the
  UAVs.
\end{abstract}
%
