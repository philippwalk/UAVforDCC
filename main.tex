...
distortion functions with minimal average distortion over $\Ome$ for a given continuous source sample
...
\philippstart \paragraph{Notation}  By $[N]=\{1,2,\dots,N\}$ we denote the first $N$ positive integers.  We will write
row vectors by bold letters and matrices by bold capital letters.  For convenience, let
$\Ball\left(\vc,r\right)=\set{\vome}{\|\vome-\vc\|^2\le r}$ be the open ball in $\R^d$ centered at $\vc\in\R^d$ with
radius $r\geq 0$.
%Note that $\Ball\left(\vc,0\right)$ is an empty set.
We denote by $\Vor^c$ the complement of the set
$\Vor\subset\R^d$. The positive numbers are denoted by $\R_+:=\set{a\in\R}{a>0}$.  Moreover, for two points
$\va,\vb\in\R^d$, we denote the generated half space between them, which contains $\va\in\R^d$ as $\HS(\va, \vb)$.
\philippend
...
%correspond to the UAVs location and the quantizer sets to the ground cells. 
...
\cite{Erdem16,GJ,GJcom18,GJ18,Erdem1,ML,MLCS,KKSS18}.  
\ifarxiv
...
distance $d$ of GT $\vome$ and UAV $\vp_n$
%
\begin{align}
...
$P_{TX,n}=\big(2^{\frac{\Rb}{B}}-1\big)B N_0d(\vq_n,(\vome,0))^{\alpha+1}10^{\frac{\psi_{dB}}{10}} (h_n \GGT
Kd^{\alpha}_0)^{-1}$.
%
...
%
The expectation  % the minimum transmitter power of the $n$th UAV to the GT is then 
%
\begin{align}
 \Dis(\vome,\vq_n):= \Expect{P_{TX,n}}& =
...
\end{align}
%
defines then the $n$th \emph{parameter distortion function} $D(\cdot,\vq_n)$  with the independent and fixed parameters
%
\begin{align}
...
density $\df$ in $\Omega$, the minimal average distortion for given \emph{parameter and quantization points}
$\vQ=(\vP,\vh)$ over all quantization sets $\Rset$ is called the \emph{average distortion} of $\vQ$
%
\begin{equation}
 \AvDis(\vQ):=\int_{\Omega}\min_{n} \left\{ \Dis(\vome,\vq_n) \right\} \df(\vome)d\vome 
  = \sum_{n=1}^N \int_{\Vor_n(\vQ)} \!\!\!\!\!\!\!D(\vome,\vq_n)\lam(\vome)d\vome=\AvDis(\vQ,\Vor(\vQ))
  \label{eq:Pbar}
\end{equation}
%
Here, the optimal quantization sets $\Rset_n^*=\Vor_n(\vQ)$ are given by \emph{generalized Voronoi regions}
$\Vor_n(\vQ)$, which are defined as the set of sample points $\vome$ with smallest distortion (distance) to the $n$th
quantization point $\vp_n$ with parameter $h_n$.  An \emph{$N-$level parameter optimized quantizer} $(\vP^*,\vh^*,\Rset^*)$ is
therefore a minimizer of the \emph{average distortion} $\AvDis$ over all parameter and quantization points
%
\begin{align}
  \min_{\vP\in\Omega^N, \bH\in\R_+^{N}} \AvDis(\vP,\bH)=\min_{\vQ\in\Qset^N} \AvDis(\vQ)\label{eq:optPbar},
\end{align}
%
which defines an \emph{$N-$facility locational-parameter optimization problem} \cite{GJ, GJcom18, GJ18}. By the
definition of the Voronoi regions \eqref{eq:Pbar} this is equivalent to the minimal average distortion over all
$N-$level parameter quantizers $(\vQ,\Rset)$, i.e., 
%
\begin{align}
 \AvDis(\vQ^*)= \min_{\vQ\in\Qset^N} \min_{\Rset\in\Ome^N} \AvDis(\vQ,\Rset) = \min_{\vQ\in\Qset^N}
  \AvDis(\vQ,\Vor(\vQ)).\label{eq:optquanteqoptdeploy}
\end{align}
%
%Although $h_n$ is a location parameter in space, the
%power is not a monotone function of the height, due to the directional antenna effect, rendering the 
%problem to a possible non-convex optimization problem.
%
%
...
s on $\Ome\times\Qset$ for arbitrary $d-$dimensional parameter sets $\Qset\subset \R^d$, which are analytical
multivariate functions  if restricted to $\Omega$.  Then the necessary condition for a local extrema is the vanishing of
the gradient at a critical point\footnote{Note, if $\nabla \Pbar$ is not continuous in $\Qset^N$ than any jump-point is
a potential critical point and has to be checked individually.}.  To derive the partial derivatives we will need to
rewrite the integral kernel, which is the minimum of $N$ continuous functions, as a sum of $N$ integrals by using
generalized Voronoi tessellations of $\Ome$.
%
...
First we will derive the tessellation of $\Ome$ by generalized Voronoi diagrams in $d$ dimensions for any parameters
$h_n\in\R_+$  associate to the quantization points $\vp_n$, which are special cases of \emph{M{\"o}bius diagrams},
...
\eqref{eq:Pbar} over all possible parameter quantization points in $\Qset^N$, i.e., we have to solve a non-convex
...
\end{align}
%
where $\Vor_n(\vP,\vh)$ are the Möbius regions given in \eqref{eq:moebius} for each fixed $\vQ$.
%
A point $\vQ^*=(\vP^*,\vh^*)$ with Möbius regions $\Vor^*=\Vor(\vP^*,\vh^*)=\{\Vor_1^*,\dots,\Vor^*_N\}$ is a critical
...
\subsection{The optimal $N-$level parameter quantizer in one-dimension}
...
\begin{remark}
  The convexity of $F(\cdot,\gam)$ can be also shown by using extensions of the Hermite-Hadamard inequality, as for example in
  \cite{ZC10}, which allows to show convexity over any interval. This suggest, that as long as the generalized Voronoi
  regions remain
...
%
\color{black}
To derive the parameter optimized quantizer, we need some general properties of the optimal regions. 
%
\philippstart
%
\begin{lemma}\label{lemma:allActive}
  Let $\Ome=[0,A]$ for some $A>0$. The $N-$level parameter optimized quantizer $\vQ^*\in(\Ome\times\R_+)^N$ for a
  uniform source density in $\Ome$ has optimal quantization regions $\Vor_n(\vQ^*)=[b^*_{n-1},b^*_n]$ with $0\leq
...
\begin{theorem*}\label{thm:commonheight}
  ...
  $(\vQ^*,\Rset^*)$ is the  uniform scalar quantizer with identical parameter values, given for $n\in[N]$  by   
  %
  \begin{align}
    \pGlob_n= \frac{A}{2N} (2n-1),\quad   h^*=h_n^*= \frac{A}{2N} g(\gam),\quad \Rset_n^*=
  ...
  %
  \begin{align}
   \AvDis(\vQ^*)= \AvDis(\vx^*,h^*)=\left(\frac{A}{2N}\right)^{2\gam-1}  \int_0^1 \frac{\big(\ome^2+g^2(\gam)\big)^{\gam}}{g(\gam)}
...
\begin{proof}
  ...
  %
  \begin{align}
    x_n^*=\frac{b_n+b_{n-1}}{2}\label{eq:xnoptimal}
  \end{align}
  %
  for $n\in[N]$.
  ...
  %
  \begin{align}
    \AvDis(\vQ^*)&=\sum_{n=1}^N \int_{b_{n-1}^*}^{b_n^*} 
  ...
  distortion for $x_n^*$ and $\Vor_n^*$. Hence, by minimizing $\AvDis(\vq^*,\tilde{\vh},\Vor^*)$ over
...
\begin{example}
We plotted in \figref{fig:goptdopt} the optimal heights and optimal average distortion for a uniform GT density in
...
\label{uniformDistortion_regions}}
\hfil
\subfloat[]{\includegraphics[width=2.8in]{HR100.eps}
\label{nonuniformDistortion_regions}}
\captionsetup{justification=justified}
\vspace{-2ex}
...
%  To bypass this non-monotonicity property  
  ...
  $\vq_n\in\Qset=\Ome\times\R_+$. The assignment rule or \emph{quantization rule} can be anything such that the regions
  ...
  sets.}). This holds for any density function $\lam(\ome)$ and target area $\Ome$.  To see this\footnote{We use the
    same argumentation as in the prove of \cite[Prop.1]{Erdem}.} let us start with any quantizer $(\vQ,\Rset)$ for
    $\Ome$ yielding to the average distortion 
  %
  \begin{align}
    \AvDis(\vQ,\Rset) &=\sum_{n=1}^N \int_{\Rset_n} \Dis(\vq_n,\ome)\lam(\ome) d\ome \geq \sum_{n=1}^N \int_{\Rset_n} 
    \left(\min_{m\in[N]} \Dis(\vq_m,\ome) \right) \lam(\ome)d\ome\\
    &=\int_{\Ome} \min_{m\in[N]} \Dis(\vq_m,\ome) \lam(\ome)d\ome
    =\sum_n \int_{\Vor_n(\vQ)} \Dis(\vq_n,\ome) \lam(\ome)d\ome\notag
  \end{align} 
  %
  where the first inequality is only achieved if for any $\ome\in \Rset_n$ we have chosen $\vq_n$ to be the optimal
  quantization point with respect to $\Dis$, or vice versa, if every $\vq_n$ is optimal for every $\ome\in \Rset_n$, which is the
  definition of the generalized Voronoi region $\Vor(\vQ)$. Therefore, minimizing over all partitions gives equality, i.e.
  %
  \begin{align}
    \min_{\Rset} \AvDis(\vQ,\Rset)=\AvDis(\vQ,\Vor(\vQ))
  \end{align}
  %
  for any $\vQ\in\Qset^N$.
  Hence, we have shown that the parameter-quantizer optimization problem is equivalent to the locational-parameter
  optimization problem
  %
  \begin{align}
    \min_{\vQ\in\Qset^N} \min_{\Rset\in\Ome^N} \AvDis(\vQ,\Rset) = \min_{\vQ\in\Qset^N}
    \AvDis(\vQ,\Vor(\vQ))=\AvDis(\vQ^*,\Vor(\vQ^*)).\label{eq:optquanteqoptdeploy2}
  \end{align}
  %
  We need to show, that for the optimal $N-$parameter-quantizer $(\vQ^*,\Vor(\vQ^*))$ we have $\mu(\Vor_n(\vQ^*))>0$ for
  all $n\in[N]$.  Let us first show that each region is indeed a closed interval, i.e.,
  $\Vor_n(\vQ^*)=[b^*_{n-1},b^*_n]$ with $0\leq b^*_{n-1}\leq b^*_n\leq A$. 
  
  By the definition of the Möbius regions in \lemref{lem:moebiusdia}, each dominance region is either a single interval (if
  it is a ball not contained in the target region or a halfspace) or two disjoint intervals (if its a ball contained in
  the target region), we can not have more than $K_n\leq 2N-2$ disjoint closed intervals for each Möbius (generalized
  Voronoi) region.  Therefore, the $n$th optimal Möbius region is given as $\Vor^*_n=\Vor_n(\vp^*,
  \vh^*)=\bigcup_{k=1}^{K_n}v_{n,k}$, where $v_{n,k}=[a_{n,k-1},a_{n,k}]$ are intervals for some $0\leq a_{n,k-1}\leq
  a_{n,k}\leq A$.
