...
Contrary to a fixed parameter selection \cite{Erdem16}, we will assign to each quantization point $\vp_n$ an independent
...
\begin{align}
  ...
  %{h_n \GGT Kd^{\alpha}_0 } \notag\\
     ...
     =:\Dis(\vome,\vq_n) \label{eq:Eptx}
\end{align}
%
and defines the $n$th \emph{distortion measure}  with the independent and fixed parameters
%
\begin{align}
...
fixed $\gam\geq 1$ a parameter dependent distortion function of $\vq_n=(\vp_n,h_n)$. Optimizing the quantizer
$(\vQ,\Rset)$ for fixed parameters  $\vh=(h_1,h_2,\dots,h_N)$  over all source samples (GT) distributed by $\df$ in
$\Omega$ yields to the \emph{average distortion}
%
\begin{equation}
...
\end{equation}
%
where we assigned for each sample $\vome$ the reproduction point with smallest distortion, which defines therefore the
corresponding quantization cells $\Vor(\vQ)=\{\Vor_n(\vQ)\}$.   An \emph{parameter optimized quantizer}
$(\vP^*,\vh^*,\Rset^*)$ is given as a
minimizer of the \emph{average distortion} $\AvDis$ over all sample points ground positions and heights
%g
\begin{align}
  \min_{\vP\in\Omega^N, \bH\in\R_+^{N}} \AvDis(\vP,\bH)\label{eq:optPbar},
\end{align}
%
which defines a \emph{locational-parameter optimization problem}. The optimal quantization sets $\Rset$ are given by the
corresponding generalized Voronoi regions $\Vor(\vQ^*,\vh^*)$, i.e., it holds
\begin{align}
  \min_{\vQ\in\Qset^N} \min_{\Rset\in\Ome^N} \AvDis(\vQ,\Rset) = \min_{\vQ\in\Qset^N}
  \AvDis(\vQ,\Vor(\vQ))=\AvDis(\vQ^*,\Vor(\vQ^*)).\label{eq:optquanteqoptdeploy}
\end{align}
%
...
analytical multivariate functions  if restricted to $\Omega$.  Then the necessary condition for a local extrema
...
non-convex \emph{$N-$facility locational-parameter optimization problem} \cite{GJ, GJcom18, GJ18} 
%
\begin{align}
