...
%
For a  set $\Omega\in\R^d$ in $d=1,2$ dimensions a quantizer is given by $N$ reproduction or
...
%An optimal deployment of $N-$UAVs is then the minimizer of the average power for all GTs in $\Ome$.


\section{Parameter Optimized Quantizers under Minimal Average Distortion}\label{Sec:OneDimension}
...
\end{align}
which we call the \emph{$N-$level parameter optimized quantizer}. % $(\vP^*,\vh^*,\Rset^*)$
%
...
%To derive the partial derivatives we will need to
...
\section{Lloyd-like Algorithms and Simulation Results over Two-Dimensions}
%
In this section, we introduce two Lloyd-like algorithms, Lloyd-A and Lloyd-B, to optimize the quantizer for two-dimensional scenarios. The proposed
...
outperform the random deployment on both uniform and non-uniform distributed target regions. From Fig.
\ref{uniformDistortion}, one can also find that the distortion achieved by Lloyd-A and Loyd-B are very close, indicating
that the optimality of the common height, as proved for the one-dimensional case in Section \ref{Sec:OneDimension}, might be extended to the two-dimensional case when the density
function is uniform.  However, one can find a non-negligible gap between Lloyd-A and Lloyd-B in Fig.
\ref{nonuniformDistortion} where the density function is non-uniform. For instance, given 16 UAVs and pass loss exponent
$\alpha=6$, Lloyd-A's distortion is 40.17 while Lloyd-B obtains a smaller distortion, 28.25, by placing UAVs at different
...
approximate to be hexagons which implies the optimality of uniform quantizer (Theorem \ref{thm:commonheight}) might be extended
...
\section{Conclusion}
%
We studied a quantizer with parameterized distortion measure with application in UAV deployment. Instead of using the traditional mean
